...
def test_creer_point_existant(mock_db_conn):
    ...
    dao_point = Dao_point()
    dao_point.existe = lambda p: (True, 1)  # Point existant
...
def test_creer_point_nouveau(mock_db_conn):
    ...
    point = Point(x=10, y=20)
    dao_point = Dao_point()
    dao_point.existe = lambda p: (False, None)  # Point non existant
...
def test_existe_point_vrai(mock_db_conn):
    ...
    assert len(mock_cursor.queries) == 1
    assert mock_cursor.queries[0][0].startswith("SELECT id_point FROM projet_2A.points")
    assert mock_cursor.queries[0][1] == {"x": 10, "y": 20}
...
def test_obtenir_point_selon_id(mock_db_conn):
    ...
    assert result.y == 20
    assert len(mock_cursor.queries) == 1
    assert mock_cursor.queries[0][0].startswith("SELECT x, y FROM projet_2A.point")
